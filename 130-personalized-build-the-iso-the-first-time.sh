...
echo
	...
	echo "Git clone the latest ArcoLinux-iso from github"
	echo
	git clone https://github.com/arcolinux/arcolinuxl-iso ./github
 
  # cp -r /usr/share/archiso/configs/releng/ ./ezreleng
   cp -r ./github/archiso/ ./ezreleng
cp pacman.conf ./ezreleng/
   ...
   rm -r ./ezreleng/syslinux
	...
	echo
	cp -rf ./personal/ ./ezreleng/airootfs/
...
############################################################################
	...
	echo
	cp -rf ./personal-twist/ ./ezreleng/airootfs/
############################################################################
############################################################################
############################################################################    
############################################################################
...
echo
	...
	echo "Deleting any files in /etc/skel"
	rm -rf $buildFolder/airootfs/etc/skel/.* 2> /dev/null
	echo

	echo "Getting the last version of bashrc in /etc/skel"
	echo
	wget https://raw.githubusercontent.com/arcolinux/arcolinux-root/master/etc/skel/.bashrc-latest -O $buildFolder/airootfs/etc/skel/.bashrc

	echo "Removing the old packages.x86_64 file from build folder"
	rm $buildFolder/packages.x86_64
	echo
	echo "Copying the new packages.x86_64 file to the build folder"
	cp -f packages.x86_64 $buildFolder/packages.x86_64
...
############################################################################
############################################################################
    cat ./personalsoftware.x86_64 >> $buildFolder/packages.x86_64
############################################################################
############################################################################
############################################################################
############################################################################
...
cp pacman.conf ./ezreleng/airootfs/etc
...
#	sed -i 's/'$oldname2'/'$newname2'/g' $buildFolder/profiledef.sh
	...
	sed -i 's/'$oldname5'/'$newname5'/g' $buildFolder/airootfs/etc/sddm.conf.d/kde_settings.conf
	#bios
	sed -i 's/'$oldname4'/'$newname4'/g' $buildFolder/syslinux/archiso_sys-linux.cfg
	#uefi
	sed -i 's/'$oldname4'/'$newname4'/g' $buildFolder/efiboot/loader/entries/1-archiso-x86_64-linux.conf
	sed -i 's/'$oldname4'/'$newname4'/g' $buildFolder/efiboot/loader/entries/2-archiso-x86_64-linux-no-nouveau.conf
	sed -i 's/'$oldname4'/'$newname4'/g' $buildFolder/efiboot/loader/entries/3-nvidianouveau.conf
	sed -i 's/'$oldname4'/'$newname4'/g' $buildFolder/efiboot/loader/entries/4-nvidianonouveau.conf
	sed -i 's/'$oldname4'/'$newname4'/g' $buildFolder/efiboot/loader/entries/5-nomodeset.conf
